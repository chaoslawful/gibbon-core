...
class Spec
{
    public static function document(string $baseUrl): array
    {
        ...
        $lessonWrite = [
            ...
            'homeworkTimeCap' => ['type' => 'string', 'nullable' => true],
            'homeworkLocation' => ['type' => 'string', 'enum' => ['Out of Class', 'In Class']],
            'homeworkSubmission' => $yn,
            'homeworkSubmissionDateOpen' => ['type' => 'string', 'nullable' => true],
            'homeworkSubmissionDrafts' => ['type' => 'string', 'nullable' => true],
            'homeworkSubmissionType' => ['type' => 'string'],
            'homeworkSubmissionRequired' => ['type' => 'string', 'nullable' => true],
            'homeworkCrowdAssess' => $yn,
            'homeworkCrowdAssessOtherTeachersRead' => $yn,
            'homeworkCrowdAssessClassmatesRead' => $yn,
            'homeworkCrowdAssessOtherStudentsRead' => $yn,
            'homeworkCrowdAssessSubmitterParentsRead' => $yn,
            'homeworkCrowdAssessClassmatesParentsRead' => $yn,
            'homeworkCrowdAssessOtherParentsRead' => $yn,
            'viewableStudents' => $yn,
            'viewableParents' => $yn,
        ...
        $spec = [
            ...
            'paths' => [
                ...
                '/v1/classes' => ['get' => [
                    'summary' => 'List classes the token can see (id, name, gibbonCourseID)',
    ...
    protected static function extraPaths(array $ok, array $created, array $noContent, array $err, callable $idParam, callable $query): array
    {
        ...
        return [
            ...
            '/v1/form-groups' => [
                'get' => ['summary' => 'List form groups', 'parameters' => [['name' => 'gibbonSchoolYearID', 'in' => 'query', 'required' => true, 'schema' => ['type' => 'string'], 'description' => 'School year id']], 'responses' => ['200' => $ok, '422' => $err]],
            ...
            '/v1/student-enrolments' => ['get' => ['summary' => 'List student enrolments', 'parameters' => [$query('gibbonSchoolYearID', 'Defaults to current year'), $query('gibbonFormGroupID', 'Filter by form group'), $query('q', 'Search name or username')], 'responses' => ['200' => $ok]]],
            ...
            '/v1/people/{id}/medical-conditions' => ['post' => [
                'summary' => 'Add a medical condition to a person (requires the medical form first)',
                'parameters' => [$idParam('id', 'gibbonPersonID')],
                'requestBody' => ['content' => ['application/json' => ['schema' => [
                    'type' => 'object',
                    'required' => ['name'],
                    'properties' => [
                        'name' => ['type' => 'string'],
                        'gibbonAlertLevelID' => ['type' => 'string'],
                        'triggers' => ['type' => 'string'],
                        'reaction' => ['type' => 'string'],
                        'response' => ['type' => 'string'],
                        'medication' => ['type' => 'string'],
                        'lastEpisode' => ['type' => 'string', 'format' => 'date'],
                        'lastEpisodeTreatment' => ['type' => 'string'],
                        'comment' => ['type' => 'string'],
                    ],
                ]]]],
                'responses' => ['201' => $created, '403' => $err, '422' => $err],
            ]],
            '/v1/person-medical-conditions/{id}' => ['delete' => [
                'summary' => 'Remove a person medical condition',
                'parameters' => [$idParam('id', 'gibbonPersonMedicalConditionID')],
                'responses' => ['204' => $noContent, '403' => $err, '404' => $err],
            ]],
            ...
            '/v1/attendance/reports/trends' => ['get' => ['summary' => 'Attendance counts by type over a date range (JSON series)', 'parameters' => [$query('dateStart', 'Start date'), $query('dateEnd', 'End date'), $query('gibbonFormGroupID', 'Filter by form group; omit for all')], 'responses' => ['200' => $ok, '403' => $err]]],
